...
Redux основан на шаблоне проектирования Flux и предоставляет предсказуемый поток данных по всему приложению. Redux имеет одно глобальное хранилище состояния, которое обновляется с помощью чистых функций (редюсеров) в ответ на действия (actions). Redux хорошо интегрируется с React, но имеет относительно высокий порог вхождения и требует много шаблонного кода.

создаем стор - объект для хранения всех состояний
оборачиваем прилоение в провайдер, и передаем в него стор (правйдер из библиотеки реакт редакс)

если это тулкит то работаем через слайсы. если обычный редакс, то через редьюсеры и экшоны
в тулките создается слайс и в него передается изначальное состояние плюс в поле редьюсер передается объект, в котором экшоны - это название полей, а значения - эту функции редьюсеры. 

редьюсер это чистая функция, которая принимает стейт и экшон. и возвращает измененный стейт
экшон - это объект с полем тайп и пэйлоад

ну идиспатчим экшоны
инфу берем из селекторов. селекторы это функции которые принимают стейт и возвращают из него какую-то инфу
