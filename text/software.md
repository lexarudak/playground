...
- **Main concepts of OOP.**
  ...
  - Наследование — потомок при создании получает все свойства и методы родителя.
  - Инкапсуляция - сокрытие внутренней логики, работа только через публичные методы
...
- **Main concepts of FP.**
  - Pure Functions - функция все возвращает один и тот же результат и не выполняет сайдефектов
  - Immutability - держим переменные неизменяемыми. при выполнении действий создаем копии
  - Higher-Order functions - функции которые могут принимать в себя другие функции как аргументы
  - Рекурсия - функция вызывающая саму себя
  - Функции первого класса - функция которая может быть передана как аргумент
  - Композиция - Композиция функций позволяет создавать новые функции из уже существующих,
- **FP vs OOP**
